...
Find it in the Admin Center: [Search.gov Home]({{ site.url }}/index.html) > [Admin Center](https://search.usa.gov/sites/) > YourSite > Manage Content > RSS
...
We use [color coding]({{ site.url }}/admin-center/color-codes.html) to indicate each feed's status. 
...
* Start with the standard form snippet on the [Code Snippets]({{ site.url }}/admin-center/activate/code.html) page under the Activate Search tab. Change the form action to `action="https://search.usa.gov/search/news` and add the following line to limit the results to your feed.<br />`<input type="hidden" name="channel" value="###">`
...
When you [provide us with your YouTube channel]({{ site.url }}/admin-center/content/youtube.html), we'll automatically index the RSS feed for your YouTube channel.
