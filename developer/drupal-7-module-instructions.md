...
This documentation will walk you through setting up the `usasearch` [Drupal 7 module](https://drupal.org/project/usasearch) field by field, to integrate your Drupal CMS with the Search.gov service. If you have trouble, take a look at our [troubleshooting tips]({{ site.url }}/developer/drupal-module-troubleshooting.html). 
...
Instructions for setting up the **Drupal 8** module are [here]({{ site.url }}/developer/drupal-8-module-instructions.html).
...
Check this box if you want to enable the Javascript for our [typeahead API]({{ site.url }}/developer/). Suggestions for popular queries will appear below your search box as searchers begin typing. A query must have been run more than 10 times in a day for it to become a search suggestion.

## Search domain

We offer [domain masking]({{ site.url }}/admin-center/display/cname.html), which allows you to display `search.example.gov` in the URL of your search results page, instead of the default `search.usa.gov`. 
...
Take a look at our [troubleshooting tips]({{ site.url }}/developer/drupal-module-troubleshooting.html), and don't hesitate to [reach out](mailto:search@support.digitalgov.gov). We're not Drupal experts ourselves, but we'll try to help.
