...
* See also our [tips on preparing for a site redesign]({{ site.url }}/indexing/redesign.html).
...
* When undergoing a [site redesign]({{ site.url }}/indexing/redesign.html), be sure to **update** Collections, Best Bets, RSS Feeds, Header and Footer links, related sites, image assets, and results page colors, as appropriate. 
...
We have a stopgap measure available to [filter certain results]({{ site.url }}/admin-center/content/domains-advanced.html) directly in the Admin Center. Under `[Search.gov Home]({{ site.url }}/) > [Admin Center](https://search.usa.gov/sites/) > YourSite > Analytics > Queries` you’ll see an option for Filter URLs. 
...
* **3rd party tracking** allows you to include search traffic in your broader website and customer experience monitoring, such as Google Analytics and Foresee.
  ...
  * **ForeSee** requires you set up a [cname]({{ site.url }}/admin-center/display/cname.html) in order for it to work (see below). 
