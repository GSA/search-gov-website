...
**What:** We will ask you about your search needs for the new site, including what domains you need to include, how you can generate an [xml sitemap]({{ site.url }}{{ site.baseurl }}/blog/sitemaps.html), and what SEO supports you’re putting in place in your new templates, like [metadata]({{ site.url }}{{ site.baseurl }}/manual/metadata.html) and [other structured elements]({{ site.url }}{{ site.baseurl }}/manual/how-search-engines-index-content-better-discoverability.html). Read more about our [indexing process here]({{ site.url }}{{ site.baseurl }}/manual/site-launch-guide.html#indexing-workflow).
...
* Ensure that each domain and subdomain you want to be searchable launches with an [xml sitemap]({{ site.url }}{{ site.baseurl }}/blog/sitemaps.html).
* Add [metadata]({{ site.url }}{{ site.baseurl }}/manual/metadata.html) blocks to the `<head>` of your page templates, and Semantic Markup to the `<body>`. 
...
**What:** Use our [Search Page Alert]({{ site.url }}{{ site.baseurl }}/manual/system-alert.html) feature to display a “pardon our dust” type message on your results page. For example:
...
**What:** Gather your [new logo and color palette]({{ site.url }}{{ site.baseurl }}/manual/brand.html), if needed. Many sites find it helpful to mock up their redesigned results page in a non-production search site - you can either [clone]({{ site.url }}{{ site.baseurl }}/manual/clone-site.html) your existing site or just use the [Add Site]({{ site.url }}{{ site.baseurl }}/manual/add-site.html) button to create a totally new one.
...
* [Domains]({{ site.url }}{{ site.baseurl }}/manual/domains.html): make sure your Domains list includes the domains and subdomains that you want included as the default content to search.
* [Collections]({{ site.url }}{{ site.baseurl }}/manual/collections.html): make sure your Collections are searching for the right content in the new location.  
* [Best Bets]({{ site.url }}{{ site.baseurl }}/manual/best-bets.html): make sure your Best Bet URLs are correct for the content’s new location.
* [Routed Queries]({{ site.url }}{{ site.baseurl }}/manual/routed-queries.html): update the target of your Routed Queries, so searchers will end up on the correct page.
* [RSS]({{ site.url }}{{ site.baseurl }}/manual/rss.html) feeds should be removed, and re-added from their new locations.
