...
* We continue to assist customers with [transitioning to HTTPS]({{ site.baseurl }}/blog/https-transition.html), adding [sites’ domain masks]({{ site.baseurl }}/admin-center/display/cname.html) to our SSL certificate and preparing to update our help manual and some backend configurations.
* When you [clone a search configuration]({{ site.baseurl }}/admin-center/dashboard/clone-site.html), your type-ahead suggestions will no longer clone to the new configuration. This enables your new site to populate type-ahead suggestions based on its own traffic.
