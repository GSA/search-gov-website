...
We’ve written a [post about robots.txt files]({{ site.url }}/indexing/robotstxt.html), talking about high level inclusion and exclusion of content from search engines. There are other key tools you will want to employ on your website to further target the content on individual pages:
...
* Leave the item in the [sitemap]({{ site.url }}/indexing/sitemaps.html), so that search engines will see the page was updated, revisit it, and see that the item should be removed from the index.
